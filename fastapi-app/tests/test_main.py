...
def test_find_by_due_fromtoday_正常系(client_fixture :TestClient):
    response = client_fixture.get("/items/today/?end=7")
    assert response.status_code == 200
    items = response.json()
    assert len(items) == 1

def test_find_by_due_fromtoday_異常系(client_fixture :TestClient):
    response = client_fixture.get("/items/today/?end=-1")
    assert response.status_code == 404
    assert response.json()["detail"] == "Task not found"
...
def test_create(client_fixture :TestClient):
    response = client_fixture.post("/items",json={"title":"kaimono3","content":"banana","due_date":"2025-11-04","completed":False})
...
def test_update_正常系(client_fixture :TestClient):
    response = client_fixture.put("/items/1",json={"title":"kaimono4","content":"apple","due_date":"2025-11-04","completed":True})
    assert response.status_code == 200
    item = response.json()
    assert item["id"] == 1
...
def test_update_異常系(client_fixture :TestClient):
    response = client_fixture.put("/items/10",json={"title":"kaimono4","content":"apple","due_date":"2025-11-04","completed":True})
    assert response.status_code == 404
    assert response.json()["detail"] == "Task not found"
